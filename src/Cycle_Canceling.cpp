...
int main(){
    ...
    while(gw.edit()) {
        ...
        // create a copy of original graph G
        H.clear();
        node_array<node> copy_in_H(G);
        node v;
        forall_nodes(v,G) copy_in_H[v] = H.new_node(v);
        edge e;
        forall_edges(e,G) H.new_edge(copy_in_H[source(e)], copy_in_H[target(e)],e);
 ...
 * while (!cycle.empty()) {
            ...
            for (int i = 0; i < cycle.size(); ++i) {
                for (int j = 0; j < cycle.size(); ++j) {
                    edge e  = search_edge(cycle[i], cycle[j]);
            ...
            /*for (size_t i = 0; i < cycle.size() - 1; ++i) {
                edge e = search_edge(cycle[i], cycle[i + 1]);
                if (e != nil) {
                    flow[e] += f;
                } else {
                    e = search_edge(cycle[i + 1], cycle[i]);
                    flow[e] -= f;
                }
            }*/
